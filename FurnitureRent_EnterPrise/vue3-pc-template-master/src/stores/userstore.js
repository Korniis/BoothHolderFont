...
export const useUserStore = defineStore('userstore', () => {
  ...
  const userinfo = reactive({
    ...
    useravatar: '', // 用户头像
    userrole:[],
    token: localStorage.getItem('enterprisetoken') || '', // 从 localStorage 获取 token（如果有）
  });
  ...
  const setToken = (token) => {
    localStorage.setItem('enterprisetoken', token); // 将 token 保存到 localStorage
  ...
  const clearUserInfo = () => {
    ...
    userinfo.useravatar = '';
    userinfo.token = '';
    localStorage.removeItem('enterprisetoken'); // 清除 localStorage 中的 token
  };
